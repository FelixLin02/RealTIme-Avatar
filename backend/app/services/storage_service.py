...
class StorageService:
    ...
    async def _delete_from_s3(self, file_url: str) -> bool:
        ...
        try:
            # 從 URL 提取檔案路徑
            filename = file_url.split(f"{settings.S3_BUCKET_NAME}.s3.{settings.AWS_REGION}.amazonaws.com/")[-1]
    ...
    async def _delete_from_gcs(self, file_url: str) -> bool:
        ...
        try:
            # 從 URL 提取檔案路徑
            filename = file_url.split(f"{settings.GCS_BUCKET_NAME}.storage.googleapis.com/")[-1]
